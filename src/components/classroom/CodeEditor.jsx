...
export default function CodeEditor({ language, onLanguageChange, code, onCodeChange, onRun, onSubmit, isRunning }) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0d1117] rounded-xl border border-slate-800/50 overflow-hidden">
      {/* Toolbar */}
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-800/50 bg-slate-950/60 flex-shrink-0">
        ...
        <div className="flex items-center gap-1">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="sm" className="h-7 text-[11px] text-slate-400 hover:text-white hover:bg-slate-800 gap-1.5 font-mono px-2.5">
      ...
      {/* Status bar / actions */}
      <div className="flex items-center justify-between px-4 py-2 border-t border-slate-800/50 bg-slate-950/60 flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            size="sm"
            className="h-7 px-3 bg-purple-600/90 hover:bg-purple-600 text-white text-[11px] font-semibold gap-1.5 shadow-sm"
          >
          ...
          <Button
            ...
            disabled={isRunning}
            size="sm"
            className="h-7 px-3 bg-emerald-600/90 hover:bg-emerald-600 text-white text-[11px] font-semibold gap-1.5 shadow-sm"
          >
            {isRunning
              ? <><Loader2 style={{ width: 11, height: 11 }} className="animate-spin" /> Running</>
              : <><Play style={{ width: 11, height: 11 }} /> Run Code</>
            }
          </Button>
          <Button
            onClick={onSubmit}
            size="sm"
            className="h-7 px-3 bg-indigo-600/90 hover:bg-indigo-600 text-white text-[11px] font-semibold gap-1.5 shadow-sm"
          >
